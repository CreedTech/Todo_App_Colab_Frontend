...
export const getUserTodos = createAsyncThunk("user/getapis", async(id, thunkAPI) => {
    // const headers = { 'X-Zapi-Auth-Token': `Bearer ${cookies.get('accessToken')}` }
    try {
...
export const getFavouriteTodos = createAsyncThunk("user/getsubscribed", async(id, thunkAPI) => {
    // const headers = { 'X-Zapi-Auth-Token': `Bearer ${cookies.get('accessToken')}` }
    try {
        const response = await fetch(`${core_url}/api/dev-platform-data/${id}`,
            // { headers }
...
export const getSubscribedUsers = createAsyncThunk("user/getSubscribedUsers", async(id, thunkAPI) => {
    // const headers = { 'X-Zapi-Auth-Token': `Bearer ${cookies.get('accessToken')}` }
    try {
        const response = await fetch(`${core_url}/api/subscriptions/${id}`,
            // { headers }
        )
        const data = await response.json()
